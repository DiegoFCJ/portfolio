...
export class NavigatorComponent implements OnInit {
  ...
  readonly availableLanguages: LanguageCode[] = ['en', 'it', 'de', 'es', 'no', 'ru'];
  ...
  tooltipTexts: Partial<Record<LanguageCode, { prev: string; next: string; theme: string; language: string }>> = {
    en: {
  ...
  themeNames: Partial<Record<LanguageCode, Record<ThemeKey, string>>> = {
  ...
  languageNames: Partial<Record<LanguageCode, Partial<Record<LanguageCode, string>>>> = {
  ...
  private readonly languageFlags: Partial<Record<LanguageCode, string>> = {
  ...
  toggleButtonLabels: Partial<Record<LanguageCode, { open: string; close: string }>> = {
  ...
  getTooltip(key: 'prev' | 'next' | 'theme' | 'language'): string {
    const dictionary = this.tooltipTexts[this.currentLang] ?? this.tooltipTexts[this.fallbackLanguage];
    return dictionary?.[key] ?? key;
  }

  getToggleButtonLabel(): string {
    const fallbackLabels = this.toggleButtonLabels[this.fallbackLanguage];
    const labels = this.toggleButtonLabels[this.currentLang] || fallbackLabels;
    const defaultOpen = fallbackLabels?.open ?? 'Open navigator';
    const defaultClose = fallbackLabels?.close ?? 'Close navigator';
    return this.isOpen ? labels?.close ?? defaultClose : labels?.open ?? defaultOpen;
  }
  ...
  getThemeName(theme: ThemeKey): string {
    const names = this.themeNames[this.currentLang] || this.themeNames[this.fallbackLanguage];
    return names?.[theme] ?? theme;
  }

  getLanguageName(language: LanguageCode): string {
    const names = this.languageNames[this.currentLang] || this.languageNames[this.fallbackLanguage];
    return names?.[language] ?? language.toUpperCase();
  }

  getLanguageFlag(language: LanguageCode): string {
    return this.languageFlags[language] ?? this.languageFlags[this.fallbackLanguage] ?? '';
  }
