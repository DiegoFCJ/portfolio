
import { SkillFullLangs } from '../dtos/SkillDTO';

export const skills: SkillFullLangs = {
    it: {
        title: 'Stack Tecnologico',
        skills: [
            {
                title: 'Back-end · Linguaggi',
                subtitle: 'Linguaggi',
                skills: [
                    {
                        name: 'Java',
                        icon: 'https://img.shields.io/badge/java-%23ED8B00.svg?style=for-the-badge&logo=openjdk&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Python',
                        icon: 'https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54',
                        clicked: false,
                    },
                    {
                        name: 'Node.js',
                        icon: 'https://img.shields.io/badge/node.js-6DA55F?style=for-the-badge&logo=node.js&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · Framework e Librerie',
                subtitle: 'Framework',
                skills: [
                    {
                        name: 'Spring Framework',
                        icon: 'https://img.shields.io/badge/spring-%236DB33F.svg?style=for-the-badge&logo=spring&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Spring Boot',
                        icon: 'https://img.shields.io/badge/Spring%20Boot-%236DB33F.svg?style=for-the-badge&logo=springboot&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Hibernate',
                        icon: 'https://img.shields.io/badge/Hibernate-59666C?style=for-the-badge&logo=Hibernate&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: '.NET',
                        icon: 'https://img.shields.io/badge/.NET-512BD4?style=for-the-badge&logo=dotnet&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · Autenticazione',
                subtitle: 'Autenticazione',
                skills: [
                    {
                        name: 'JWT',
                        icon: 'https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=JSON%20web%20tokens&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Argon2',
                        icon: 'https://img.shields.io/badge/Argon2-4B275F?style=for-the-badge&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · Integrazione e Automazione',
                subtitle: 'Piattaforme di integrazione, CRM e osservabilità',
                skills: [
                    {
                        name: 'Boomi',
                        icon: 'https://img.shields.io/badge/Boomi-1E90FF?style=for-the-badge&logo=boomi&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Salesforce',
                        icon: 'https://img.shields.io/badge/Salesforce-00A1E0?style=for-the-badge&logo=salesforce&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Elastic Stack',
                        icon: 'https://img.shields.io/badge/Elastic-005571?style=for-the-badge&logo=elastic&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · Database',
                subtitle: 'Database',
                skills: [
                    {
                        name: 'MySQL',
                        icon: 'https://img.shields.io/badge/mysql-4479A1.svg?style=for-the-badge&logo=mysql&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'PostgreSQL',
                        icon: 'https://img.shields.io/badge/postgreSQL-%23316192.svg?style=for-the-badge&logo=postgresql&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Oracle Database',
                        icon: 'https://img.shields.io/badge/OracleDB-F80000.svg?style=for-the-badge&logo=oracle&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · Test API',
                subtitle: 'Test, mock e documentazione dei servizi',
                skills: [
                    {
                        name: 'Swagger',
                        icon: 'https://img.shields.io/badge/Swagger-85EA2D?style=for-the-badge&logo=swagger&logoColor=black',
                        clicked: false,
                    },
                    {
                        name: 'Postman',
                        icon: 'https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Front-end · Linguaggi e Fondamenta',
                subtitle: 'Linguaggi e markup',
                skills: [
                    {
                        name: 'JavaScript',
                        icon: 'https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E',
                        clicked: false,
                    },
                    {
                        name: 'TypeScript',
                        icon: 'https://img.shields.io/badge/typescript-%23007ACC.svg?style=for-the-badge&logo=typescript&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'HTML5',
                        icon: 'https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'CSS3',
                        icon: 'https://img.shields.io/badge/CSS-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Front-end · Framework e Librerie',
                subtitle: 'Framework',
                skills: [
                    {
                        name: 'Angular',
                        icon: 'https://img.shields.io/badge/angular-%23DD0031.svg?style=for-the-badge&logo=angular&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'AngularJS',
                        icon: 'https://img.shields.io/badge/AngularJS-E23237?style=for-the-badge&logo=angularjs&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'React',
                        icon: 'https://img.shields.io/badge/react-%2320232a.svg?style=for-the-badge&logo=react&logoColor=%2361DAFB',
                        clicked: false,
                    },
                    {
                        name: 'Next.js',
                        icon: 'https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Front-end · UI e Styling',
                subtitle: 'Componenti e strumenti per design responsive',
                skills: [
                    {
                        name: 'SCSS',
                        icon: 'https://img.shields.io/badge/SCSS-hotpink.svg?style=for-the-badge&logo=sass&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Bootstrap',
                        icon: 'https://img.shields.io/badge/bootstrap-%238511FA.svg?style=for-the-badge&logo=bootstrap&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Tailwind CSS',
                        icon: 'https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'SweetAlert2',
                        icon: 'https://img.shields.io/badge/SweetAlert2-3085d6?style=for-the-badge&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Front-end · UI Java',
                subtitle: 'Tecnologie Java per interfacce grafiche',
                skills: [
                    {
                        name: 'JavaFX',
                        icon: 'https://img.shields.io/badge/JavaFX-3D8E9C?style=for-the-badge&logo=openjdk&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'JSP',
                        icon: 'https://img.shields.io/badge/JSP-007396?style=for-the-badge&logo=java&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Containerizzazione',
                subtitle: 'Containerizzazione e orchestrazione',
                skills: [
                    {
                        name: 'Docker',
                        icon: 'https://img.shields.io/badge/docker-%230db7ed.svg?style=for-the-badge&logo=docker&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Kubernetes',
                        icon: 'https://img.shields.io/badge/kubernetes-%23326ce5.svg?style=for-the-badge&logo=kubernetes&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'OpenShift',
                        icon: 'https://img.shields.io/badge/OpenShift-EE0000.svg?style=for-the-badge&logo=redhatopenshift&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · CI/CD',
                subtitle: 'Automazione delle build e pipeline di delivery',
                skills: [
                    {
                        name: 'Gradle',
                        icon: 'https://img.shields.io/badge/Gradle-02303A.svg?style=for-the-badge&logo=gradle&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Apache Maven',
                        icon: 'https://img.shields.io/badge/Apache%20Maven-C71A36?style=for-the-badge&logo=apachemaven&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Platform Services',
                subtitle: 'Hosting PaaS e BaaS gestiti',
                skills: [
                    {
                        name: 'Heroku',
                        icon: 'https://img.shields.io/badge/Heroku-430098?style=for-the-badge&logo=heroku&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Firebase',
                        icon: 'https://img.shields.io/badge/firebase-ffca28?style=for-the-badge&logo=firebase&logoColor=black',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Script e Configurazione',
                subtitle: 'Automazione infrastrutturale e shell script',
                skills: [
                    {
                        name: 'YAML',
                        icon: 'https://img.shields.io/badge/YAML-CB171E?style=for-the-badge&logo=yaml&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Bash',
                        icon: 'https://img.shields.io/badge/Bash-4EAA25?style=for-the-badge&logo=gnu-bash&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Shell Script (.sh)',
                        icon: 'https://img.shields.io/badge/Shell_Script-121011?style=for-the-badge&logo=gnu-bash&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Versioning',
                subtitle: 'Versioning distribuito e repository',
                skills: [
                    {
                        name: 'Git',
                        icon: 'https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'GitHub',
                        icon: 'https://img.shields.io/badge/github-%23121011.svg?style=for-the-badge&logo=github&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'GitLab',
                        icon: 'https://img.shields.io/badge/gitlab-%23181717.svg?style=for-the-badge&logo=gitlab&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Bitbucket',
                        icon: 'https://img.shields.io/badge/Bitbucket-%230047B3.svg?style=for-the-badge&logo=bitbucket&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Tooling · Pianificazione',
                subtitle: 'Pianificazione',
                skills: [
                    {
                        name: 'Trello',
                        icon: 'https://img.shields.io/badge/Trello-%23026AA7.svg?style=for-the-badge&logo=Trello&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Jira',
                        icon: 'https://img.shields.io/badge/jira-%230A0FFF.svg?style=for-the-badge&logo=jira&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Notion',
                        icon: 'https://img.shields.io/badge/Notion-000000.svg?style=for-the-badge&logo=notion&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Tooling · Produttività e Design',
                subtitle: 'Suite creative e strumenti d’ufficio',
                skills: [
                    {
                        name: 'Adobe Photoshop',
                        icon: 'https://img.shields.io/badge/Adobe%20Photoshop-31A8FF?style=for-the-badge&logo=adobephotoshop&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Adobe Illustrator',
                        icon: 'https://img.shields.io/badge/Adobe%20Illustrator-FF9A00?style=for-the-badge&logo=adobeillustrator&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Microsoft Office',
                        icon: 'https://img.shields.io/badge/Microsoft%20Office-D83B01?style=for-the-badge&logo=microsoftoffice&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Microsoft Excel',
                        icon: 'https://img.shields.io/badge/Microsoft%20Excel-217346?style=for-the-badge&logo=microsoftexcel&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Sistemi Operativi',
                subtitle: 'Sistemi Operativi',
                skills: [
                    {
                        name: 'Fedora',
                        icon: 'https://img.shields.io/badge/Fedora-294172?style=for-the-badge&logo=fedora&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Ubuntu',
                        icon: 'https://img.shields.io/badge/Ubuntu-E95420?style=for-the-badge&logo=ubuntu&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Windows',
                        icon: 'https://img.shields.io/badge/Windows-0078D6?style=for-the-badge&logo=windows&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Linux',
                        icon: 'https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black',
                        clicked: false,
                    },
                ],
            },
        ],
    },
    en: {
        title: 'Tech Stack',
        skills: [
            {
                title: 'Back-end · Languages',
                subtitle: 'Languages',
                skills: [
                    {
                        name: 'Java',
                        icon: 'https://img.shields.io/badge/java-%23ED8B00.svg?style=for-the-badge&logo=openjdk&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Python',
                        icon: 'https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54',
                        clicked: false,
                    },
                    {
                        name: 'Node.js',
                        icon: 'https://img.shields.io/badge/node.js-6DA55F?style=for-the-badge&logo=node.js&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · Frameworks & Libraries',
                subtitle: 'Frameworks',
                skills: [
                    {
                        name: 'Spring Framework',
                        icon: 'https://img.shields.io/badge/spring-%236DB33F.svg?style=for-the-badge&logo=spring&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Spring Boot',
                        icon: 'https://img.shields.io/badge/Spring%20Boot-%236DB33F.svg?style=for-the-badge&logo=springboot&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Hibernate',
                        icon: 'https://img.shields.io/badge/Hibernate-59666C?style=for-the-badge&logo=Hibernate&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: '.NET',
                        icon: 'https://img.shields.io/badge/.NET-512BD4?style=for-the-badge&logo=dotnet&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · Authentication',
                subtitle: 'Authentication',
                skills: [
                    {
                        name: 'JWT',
                        icon: 'https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=JSON%20web%20tokens&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Argon2',
                        icon: 'https://img.shields.io/badge/Argon2-4B275F?style=for-the-badge&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · Integration & Automation',
                subtitle: 'Integration platforms, CRM, and observability',
                skills: [
                    {
                        name: 'Boomi',
                        icon: 'https://img.shields.io/badge/Boomi-1E90FF?style=for-the-badge&logo=boomi&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Salesforce',
                        icon: 'https://img.shields.io/badge/Salesforce-00A1E0?style=for-the-badge&logo=salesforce&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Elastic Stack',
                        icon: 'https://img.shields.io/badge/Elastic-005571?style=for-the-badge&logo=elastic&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · Databases',
                subtitle: 'Databases',
                skills: [
                    {
                        name: 'MySQL',
                        icon: 'https://img.shields.io/badge/mysql-4479A1.svg?style=for-the-badge&logo=mysql&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'PostgreSQL',
                        icon: 'https://img.shields.io/badge/postgreSQL-%23316192.svg?style=for-the-badge&logo=postgresql&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Oracle Database',
                        icon: 'https://img.shields.io/badge/OracleDB-F80000.svg?style=for-the-badge&logo=oracle&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · API Testing',
                subtitle: 'Testing, mocking, and documenting services',
                skills: [
                    {
                        name: 'Swagger',
                        icon: 'https://img.shields.io/badge/Swagger-85EA2D?style=for-the-badge&logo=swagger&logoColor=black',
                        clicked: false,
                    },
                    {
                        name: 'Postman',
                        icon: 'https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Front-end · Languages & Foundations',
                subtitle: 'Languages and markup',
                skills: [
                    {
                        name: 'JavaScript',
                        icon: 'https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E',
                        clicked: false,
                    },
                    {
                        name: 'TypeScript',
                        icon: 'https://img.shields.io/badge/typescript-%23007ACC.svg?style=for-the-badge&logo=typescript&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'HTML5',
                        icon: 'https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'CSS3',
                        icon: 'https://img.shields.io/badge/CSS-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Front-end · Frameworks & Libraries',
                subtitle: 'Frameworks',
                skills: [
                    {
                        name: 'Angular',
                        icon: 'https://img.shields.io/badge/angular-%23DD0031.svg?style=for-the-badge&logo=angular&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'AngularJS',
                        icon: 'https://img.shields.io/badge/AngularJS-E23237?style=for-the-badge&logo=angularjs&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'React',
                        icon: 'https://img.shields.io/badge/react-%2320232a.svg?style=for-the-badge&logo=react&logoColor=%2361DAFB',
                        clicked: false,
                    },
                    {
                        name: 'Next.js',
                        icon: 'https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Front-end · UI & Styling',
                subtitle: 'Components and tools for responsive design',
                skills: [
                    {
                        name: 'SCSS',
                        icon: 'https://img.shields.io/badge/SCSS-hotpink.svg?style=for-the-badge&logo=sass&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Bootstrap',
                        icon: 'https://img.shields.io/badge/bootstrap-%238511FA.svg?style=for-the-badge&logo=bootstrap&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Tailwind CSS',
                        icon: 'https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'SweetAlert2',
                        icon: 'https://img.shields.io/badge/SweetAlert2-3085d6?style=for-the-badge&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Front-end · Java UI',
                subtitle: 'Java technologies for graphical interfaces',
                skills: [
                    {
                        name: 'JavaFX',
                        icon: 'https://img.shields.io/badge/JavaFX-3D8E9C?style=for-the-badge&logo=openjdk&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'JSP',
                        icon: 'https://img.shields.io/badge/JSP-007396?style=for-the-badge&logo=java&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Containerisation',
                subtitle: 'Containerisation and orchestration',
                skills: [
                    {
                        name: 'Docker',
                        icon: 'https://img.shields.io/badge/docker-%230db7ed.svg?style=for-the-badge&logo=docker&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Kubernetes',
                        icon: 'https://img.shields.io/badge/kubernetes-%23326ce5.svg?style=for-the-badge&logo=kubernetes&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'OpenShift',
                        icon: 'https://img.shields.io/badge/OpenShift-EE0000.svg?style=for-the-badge&logo=redhatopenshift&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · CI/CD',
                subtitle: 'Build automation and continuous delivery pipelines',
                skills: [
                    {
                        name: 'Gradle',
                        icon: 'https://img.shields.io/badge/Gradle-02303A.svg?style=for-the-badge&logo=gradle&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Apache Maven',
                        icon: 'https://img.shields.io/badge/Apache%20Maven-C71A36?style=for-the-badge&logo=apachemaven&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Platform Services',
                subtitle: 'Managed PaaS and BaaS hosting',
                skills: [
                    {
                        name: 'Heroku',
                        icon: 'https://img.shields.io/badge/Heroku-430098?style=for-the-badge&logo=heroku&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Firebase',
                        icon: 'https://img.shields.io/badge/firebase-ffca28?style=for-the-badge&logo=firebase&logoColor=black',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Scripts & Configuration',
                subtitle: 'Infrastructure automation and shell scripts',
                skills: [
                    {
                        name: 'YAML',
                        icon: 'https://img.shields.io/badge/YAML-CB171E?style=for-the-badge&logo=yaml&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Bash',
                        icon: 'https://img.shields.io/badge/Bash-4EAA25?style=for-the-badge&logo=gnu-bash&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Shell Script (.sh)',
                        icon: 'https://img.shields.io/badge/Shell_Script-121011?style=for-the-badge&logo=gnu-bash&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Versioning',
                subtitle: 'Distributed version control and repositories',
                skills: [
                    {
                        name: 'Git',
                        icon: 'https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'GitHub',
                        icon: 'https://img.shields.io/badge/github-%23121011.svg?style=for-the-badge&logo=github&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'GitLab',
                        icon: 'https://img.shields.io/badge/gitlab-%23181717.svg?style=for-the-badge&logo=gitlab&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Bitbucket',
                        icon: 'https://img.shields.io/badge/Bitbucket-%230047B3.svg?style=for-the-badge&logo=bitbucket&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Tooling · Planning',
                subtitle: 'Planning',
                skills: [
                    {
                        name: 'Trello',
                        icon: 'https://img.shields.io/badge/Trello-%23026AA7.svg?style=for-the-badge&logo=Trello&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Jira',
                        icon: 'https://img.shields.io/badge/jira-%230A0FFF.svg?style=for-the-badge&logo=jira&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Notion',
                        icon: 'https://img.shields.io/badge/Notion-000000.svg?style=for-the-badge&logo=notion&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Tooling · Productivity & Design',
                subtitle: 'Creative suites and office tooling',
                skills: [
                    {
                        name: 'Adobe Photoshop',
                        icon: 'https://img.shields.io/badge/Adobe%20Photoshop-31A8FF?style=for-the-badge&logo=adobephotoshop&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Adobe Illustrator',
                        icon: 'https://img.shields.io/badge/Adobe%20Illustrator-FF9A00?style=for-the-badge&logo=adobeillustrator&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Microsoft Office',
                        icon: 'https://img.shields.io/badge/Microsoft%20Office-D83B01?style=for-the-badge&logo=microsoftoffice&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Microsoft Excel',
                        icon: 'https://img.shields.io/badge/Microsoft%20Excel-217346?style=for-the-badge&logo=microsoftexcel&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Operating Systems',
                subtitle: 'Operating Systems',
                skills: [
                    {
                        name: 'Fedora',
                        icon: 'https://img.shields.io/badge/Fedora-294172?style=for-the-badge&logo=fedora&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Ubuntu',
                        icon: 'https://img.shields.io/badge/Ubuntu-E95420?style=for-the-badge&logo=ubuntu&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Windows',
                        icon: 'https://img.shields.io/badge/Windows-0078D6?style=for-the-badge&logo=windows&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Linux',
                        icon: 'https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black',
                        clicked: false,
                    },
                ],
            },
        ],
    },
    no: {
        title: 'Teknologisk stack',
        skills: [
            {
                title: 'Back-end · Språk',
                subtitle: 'Språk',
                skills: [
                    {
                        name: 'Java',
                        icon: 'https://img.shields.io/badge/java-%23ED8B00.svg?style=for-the-badge&logo=openjdk&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Python',
                        icon: 'https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54',
                        clicked: false,
                    },
                    {
                        name: 'Node.js',
                        icon: 'https://img.shields.io/badge/node.js-6DA55F?style=for-the-badge&logo=node.js&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · Rammeverk og biblioteker',
                subtitle: 'Rammeverk',
                skills: [
                    {
                        name: 'Spring Framework',
                        icon: 'https://img.shields.io/badge/spring-%236DB33F.svg?style=for-the-badge&logo=spring&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Spring Boot',
                        icon: 'https://img.shields.io/badge/Spring%20Boot-%236DB33F.svg?style=for-the-badge&logo=springboot&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Hibernate',
                        icon: 'https://img.shields.io/badge/Hibernate-59666C?style=for-the-badge&logo=Hibernate&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: '.NET',
                        icon: 'https://img.shields.io/badge/.NET-512BD4?style=for-the-badge&logo=dotnet&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · Autentisering',
                subtitle: 'Autentisering',
                skills: [
                    {
                        name: 'JWT',
                        icon: 'https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=JSON%20web%20tokens&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Argon2',
                        icon: 'https://img.shields.io/badge/Argon2-4B275F?style=for-the-badge&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · Integrasjon og automatisering',
                subtitle: 'Integrasjonsplattformer, CRM og observabilitet',
                skills: [
                    {
                        name: 'Boomi',
                        icon: 'https://img.shields.io/badge/Boomi-1E90FF?style=for-the-badge&logo=boomi&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Salesforce',
                        icon: 'https://img.shields.io/badge/Salesforce-00A1E0?style=for-the-badge&logo=salesforce&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Elastic Stack',
                        icon: 'https://img.shields.io/badge/Elastic-005571?style=for-the-badge&logo=elastic&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · Databaser',
                subtitle: 'Databaser',
                skills: [
                    {
                        name: 'MySQL',
                        icon: 'https://img.shields.io/badge/mysql-4479A1.svg?style=for-the-badge&logo=mysql&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'PostgreSQL',
                        icon: 'https://img.shields.io/badge/postgreSQL-%23316192.svg?style=for-the-badge&logo=postgresql&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Oracle Database',
                        icon: 'https://img.shields.io/badge/OracleDB-F80000.svg?style=for-the-badge&logo=oracle&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · API-testing',
                subtitle: 'Testing, mocking og dokumentasjon av tjenester',
                skills: [
                    {
                        name: 'Swagger',
                        icon: 'https://img.shields.io/badge/Swagger-85EA2D?style=for-the-badge&logo=swagger&logoColor=black',
                        clicked: false,
                    },
                    {
                        name: 'Postman',
                        icon: 'https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Front-end · Språk og grunnlag',
                subtitle: 'Språk og markup',
                skills: [
                    {
                        name: 'JavaScript',
                        icon: 'https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E',
                        clicked: false,
                    },
                    {
                        name: 'TypeScript',
                        icon: 'https://img.shields.io/badge/typescript-%23007ACC.svg?style=for-the-badge&logo=typescript&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'HTML5',
                        icon: 'https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'CSS3',
                        icon: 'https://img.shields.io/badge/CSS-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Front-end · Rammeverk og biblioteker',
                subtitle: 'Rammeverk',
                skills: [
                    {
                        name: 'Angular',
                        icon: 'https://img.shields.io/badge/angular-%23DD0031.svg?style=for-the-badge&logo=angular&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'AngularJS',
                        icon: 'https://img.shields.io/badge/AngularJS-E23237?style=for-the-badge&logo=angularjs&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'React',
                        icon: 'https://img.shields.io/badge/react-%2320232a.svg?style=for-the-badge&logo=react&logoColor=%2361DAFB',
                        clicked: false,
                    },
                    {
                        name: 'Next.js',
                        icon: 'https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Front-end · UI og styling',
                subtitle: 'Komponenter og verktøy for responsivt design',
                skills: [
                    {
                        name: 'SCSS',
                        icon: 'https://img.shields.io/badge/SCSS-hotpink.svg?style=for-the-badge&logo=sass&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Bootstrap',
                        icon: 'https://img.shields.io/badge/bootstrap-%238511FA.svg?style=for-the-badge&logo=bootstrap&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Tailwind CSS',
                        icon: 'https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'SweetAlert2',
                        icon: 'https://img.shields.io/badge/SweetAlert2-3085d6?style=for-the-badge&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Front-end · UI i Java',
                subtitle: 'Java-teknologier for grafiske grensesnitt',
                skills: [
                    {
                        name: 'JavaFX',
                        icon: 'https://img.shields.io/badge/JavaFX-3D8E9C?style=for-the-badge&logo=openjdk&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'JSP',
                        icon: 'https://img.shields.io/badge/JSP-007396?style=for-the-badge&logo=java&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Containerisering',
                subtitle: 'Containerisering og orkestrering',
                skills: [
                    {
                        name: 'Docker',
                        icon: 'https://img.shields.io/badge/docker-%230db7ed.svg?style=for-the-badge&logo=docker&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Kubernetes',
                        icon: 'https://img.shields.io/badge/kubernetes-%23326ce5.svg?style=for-the-badge&logo=kubernetes&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'OpenShift',
                        icon: 'https://img.shields.io/badge/OpenShift-EE0000.svg?style=for-the-badge&logo=redhatopenshift&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · CI/CD',
                subtitle: 'Automatisering av bygg og leveranse-pipelines',
                skills: [
                    {
                        name: 'Gradle',
                        icon: 'https://img.shields.io/badge/Gradle-02303A.svg?style=for-the-badge&logo=gradle&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Apache Maven',
                        icon: 'https://img.shields.io/badge/Apache%20Maven-C71A36?style=for-the-badge&logo=apachemaven&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Plattformtjenester',
                subtitle: 'Administrerte PaaS- og BaaS-løsninger',
                skills: [
                    {
                        name: 'Heroku',
                        icon: 'https://img.shields.io/badge/Heroku-430098?style=for-the-badge&logo=heroku&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Firebase',
                        icon: 'https://img.shields.io/badge/firebase-ffca28?style=for-the-badge&logo=firebase&logoColor=black',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Skript og konfigurasjon',
                subtitle: 'Infrastruktur-automatisering og skallskript',
                skills: [
                    {
                        name: 'YAML',
                        icon: 'https://img.shields.io/badge/YAML-CB171E?style=for-the-badge&logo=yaml&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Bash',
                        icon: 'https://img.shields.io/badge/Bash-4EAA25?style=for-the-badge&logo=gnu-bash&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Shell Script (.sh)',
                        icon: 'https://img.shields.io/badge/Shell_Script-121011?style=for-the-badge&logo=gnu-bash&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Versjonskontroll',
                subtitle: 'Distribuert versjonskontroll og repositories',
                skills: [
                    {
                        name: 'Git',
                        icon: 'https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'GitHub',
                        icon: 'https://img.shields.io/badge/github-%23121011.svg?style=for-the-badge&logo=github&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'GitLab',
                        icon: 'https://img.shields.io/badge/gitlab-%23181717.svg?style=for-the-badge&logo=gitlab&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Bitbucket',
                        icon: 'https://img.shields.io/badge/Bitbucket-%230047B3.svg?style=for-the-badge&logo=bitbucket&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Verktøy · Planlegging',
                subtitle: 'Planlegging',
                skills: [
                    {
                        name: 'Trello',
                        icon: 'https://img.shields.io/badge/Trello-%23026AA7.svg?style=for-the-badge&logo=Trello&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Jira',
                        icon: 'https://img.shields.io/badge/jira-%230A0FFF.svg?style=for-the-badge&logo=jira&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Notion',
                        icon: 'https://img.shields.io/badge/Notion-000000.svg?style=for-the-badge&logo=notion&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Verktøy · Produktivitet og design',
                subtitle: 'Kreativ programvare og kontorverktøy',
                skills: [
                    {
                        name: 'Adobe Photoshop',
                        icon: 'https://img.shields.io/badge/Adobe%20Photoshop-31A8FF?style=for-the-badge&logo=adobephotoshop&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Adobe Illustrator',
                        icon: 'https://img.shields.io/badge/Adobe%20Illustrator-FF9A00?style=for-the-badge&logo=adobeillustrator&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Microsoft Office',
                        icon: 'https://img.shields.io/badge/Microsoft%20Office-D83B01?style=for-the-badge&logo=microsoftoffice&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Microsoft Excel',
                        icon: 'https://img.shields.io/badge/Microsoft%20Excel-217346?style=for-the-badge&logo=microsoftexcel&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Operativsystemer',
                subtitle: 'Operativsystemer',
                skills: [
                    {
                        name: 'Fedora',
                        icon: 'https://img.shields.io/badge/Fedora-294172?style=for-the-badge&logo=fedora&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Ubuntu',
                        icon: 'https://img.shields.io/badge/Ubuntu-E95420?style=for-the-badge&logo=ubuntu&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Windows',
                        icon: 'https://img.shields.io/badge/Windows-0078D6?style=for-the-badge&logo=windows&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Linux',
                        icon: 'https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black',
                        clicked: false,
                    },
                ],
            },
        ],
    },
    ru: {
        title: 'Технологический стек',
        skills: [
            {
                title: 'Back-end · Языки',
                subtitle: 'Языки',
                skills: [
                    {
                        name: 'Java',
                        icon: 'https://img.shields.io/badge/java-%23ED8B00.svg?style=for-the-badge&logo=openjdk&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Python',
                        icon: 'https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54',
                        clicked: false,
                    },
                    {
                        name: 'Node.js',
                        icon: 'https://img.shields.io/badge/node.js-6DA55F?style=for-the-badge&logo=node.js&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · Фреймворки и библиотеки',
                subtitle: 'Фреймворки',
                skills: [
                    {
                        name: 'Spring Framework',
                        icon: 'https://img.shields.io/badge/spring-%236DB33F.svg?style=for-the-badge&logo=spring&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Spring Boot',
                        icon: 'https://img.shields.io/badge/Spring%20Boot-%236DB33F.svg?style=for-the-badge&logo=springboot&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Hibernate',
                        icon: 'https://img.shields.io/badge/Hibernate-59666C?style=for-the-badge&logo=Hibernate&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: '.NET',
                        icon: 'https://img.shields.io/badge/.NET-512BD4?style=for-the-badge&logo=dotnet&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · Аутентификация',
                subtitle: 'Аутентификация',
                skills: [
                    {
                        name: 'JWT',
                        icon: 'https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=JSON%20web%20tokens&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Argon2',
                        icon: 'https://img.shields.io/badge/Argon2-4B275F?style=for-the-badge&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · Интеграция и автоматизация',
                subtitle: 'Платформы интеграции, CRM и наблюдаемость',
                skills: [
                    {
                        name: 'Boomi',
                        icon: 'https://img.shields.io/badge/Boomi-1E90FF?style=for-the-badge&logo=boomi&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Salesforce',
                        icon: 'https://img.shields.io/badge/Salesforce-00A1E0?style=for-the-badge&logo=salesforce&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Elastic Stack',
                        icon: 'https://img.shields.io/badge/Elastic-005571?style=for-the-badge&logo=elastic&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · Базы данных',
                subtitle: 'Базы данных',
                skills: [
                    {
                        name: 'MySQL',
                        icon: 'https://img.shields.io/badge/mysql-4479A1.svg?style=for-the-badge&logo=mysql&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'PostgreSQL',
                        icon: 'https://img.shields.io/badge/postgreSQL-%23316192.svg?style=for-the-badge&logo=postgresql&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Oracle Database',
                        icon: 'https://img.shields.io/badge/OracleDB-F80000.svg?style=for-the-badge&logo=oracle&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Back-end · Тестирование API',
                subtitle: 'Тестирование, мокирование и документация сервисов',
                skills: [
                    {
                        name: 'Swagger',
                        icon: 'https://img.shields.io/badge/Swagger-85EA2D?style=for-the-badge&logo=swagger&logoColor=black',
                        clicked: false,
                    },
                    {
                        name: 'Postman',
                        icon: 'https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Front-end · Языки и основы',
                subtitle: 'Языки и разметка',
                skills: [
                    {
                        name: 'JavaScript',
                        icon: 'https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E',
                        clicked: false,
                    },
                    {
                        name: 'TypeScript',
                        icon: 'https://img.shields.io/badge/typescript-%23007ACC.svg?style=for-the-badge&logo=typescript&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'HTML5',
                        icon: 'https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'CSS3',
                        icon: 'https://img.shields.io/badge/CSS-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Front-end · Фреймворки и библиотеки',
                subtitle: 'Фреймворки',
                skills: [
                    {
                        name: 'Angular',
                        icon: 'https://img.shields.io/badge/angular-%23DD0031.svg?style=for-the-badge&logo=angular&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'AngularJS',
                        icon: 'https://img.shields.io/badge/AngularJS-E23237?style=for-the-badge&logo=angularjs&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'React',
                        icon: 'https://img.shields.io/badge/react-%2320232a.svg?style=for-the-badge&logo=react&logoColor=%2361DAFB',
                        clicked: false,
                    },
                    {
                        name: 'Next.js',
                        icon: 'https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Front-end · UI и стилизация',
                subtitle: 'Компоненты и инструменты для адаптивного дизайна',
                skills: [
                    {
                        name: 'SCSS',
                        icon: 'https://img.shields.io/badge/SCSS-hotpink.svg?style=for-the-badge&logo=sass&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Bootstrap',
                        icon: 'https://img.shields.io/badge/bootstrap-%238511FA.svg?style=for-the-badge&logo=bootstrap&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Tailwind CSS',
                        icon: 'https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'SweetAlert2',
                        icon: 'https://img.shields.io/badge/SweetAlert2-3085d6?style=for-the-badge&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Front-end · UI на Java',
                subtitle: 'Java-технологии для графических интерфейсов',
                skills: [
                    {
                        name: 'JavaFX',
                        icon: 'https://img.shields.io/badge/JavaFX-3D8E9C?style=for-the-badge&logo=openjdk&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'JSP',
                        icon: 'https://img.shields.io/badge/JSP-007396?style=for-the-badge&logo=java&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Контейнеризация',
                subtitle: 'Контейнеризация и оркестрация',
                skills: [
                    {
                        name: 'Docker',
                        icon: 'https://img.shields.io/badge/docker-%230db7ed.svg?style=for-the-badge&logo=docker&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Kubernetes',
                        icon: 'https://img.shields.io/badge/kubernetes-%23326ce5.svg?style=for-the-badge&logo=kubernetes&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'OpenShift',
                        icon: 'https://img.shields.io/badge/OpenShift-EE0000.svg?style=for-the-badge&logo=redhatopenshift&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · CI/CD',
                subtitle: 'Автоматизация сборок и delivery-пайплайнов',
                skills: [
                    {
                        name: 'Gradle',
                        icon: 'https://img.shields.io/badge/Gradle-02303A.svg?style=for-the-badge&logo=gradle&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Apache Maven',
                        icon: 'https://img.shields.io/badge/Apache%20Maven-C71A36?style=for-the-badge&logo=apachemaven&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Платформенные сервисы',
                subtitle: 'Управляемые PaaS- и BaaS-сервисы',
                skills: [
                    {
                        name: 'Heroku',
                        icon: 'https://img.shields.io/badge/Heroku-430098?style=for-the-badge&logo=heroku&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Firebase',
                        icon: 'https://img.shields.io/badge/firebase-ffca28?style=for-the-badge&logo=firebase&logoColor=black',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Скрипты и конфигурация',
                subtitle: 'Автоматизация инфраструктуры и shell-скрипты',
                skills: [
                    {
                        name: 'YAML',
                        icon: 'https://img.shields.io/badge/YAML-CB171E?style=for-the-badge&logo=yaml&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Bash',
                        icon: 'https://img.shields.io/badge/Bash-4EAA25?style=for-the-badge&logo=gnu-bash&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Shell Script (.sh)',
                        icon: 'https://img.shields.io/badge/Shell_Script-121011?style=for-the-badge&logo=gnu-bash&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Версионирование',
                subtitle: 'Распределённый контроль версий и репозитории',
                skills: [
                    {
                        name: 'Git',
                        icon: 'https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'GitHub',
                        icon: 'https://img.shields.io/badge/github-%23121011.svg?style=for-the-badge&logo=github&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'GitLab',
                        icon: 'https://img.shields.io/badge/gitlab-%23181717.svg?style=for-the-badge&logo=gitlab&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Bitbucket',
                        icon: 'https://img.shields.io/badge/Bitbucket-%230047B3.svg?style=for-the-badge&logo=bitbucket&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Инструменты · Планирование',
                subtitle: 'Планирование',
                skills: [
                    {
                        name: 'Trello',
                        icon: 'https://img.shields.io/badge/Trello-%23026AA7.svg?style=for-the-badge&logo=Trello&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Jira',
                        icon: 'https://img.shields.io/badge/jira-%230A0FFF.svg?style=for-the-badge&logo=jira&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Notion',
                        icon: 'https://img.shields.io/badge/Notion-000000.svg?style=for-the-badge&logo=notion&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'Инструменты · Производительность и дизайн',
                subtitle: 'Креативные пакеты и офисные инструменты',
                skills: [
                    {
                        name: 'Adobe Photoshop',
                        icon: 'https://img.shields.io/badge/Adobe%20Photoshop-31A8FF?style=for-the-badge&logo=adobephotoshop&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Adobe Illustrator',
                        icon: 'https://img.shields.io/badge/Adobe%20Illustrator-FF9A00?style=for-the-badge&logo=adobeillustrator&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Microsoft Office',
                        icon: 'https://img.shields.io/badge/Microsoft%20Office-D83B01?style=for-the-badge&logo=microsoftoffice&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Microsoft Excel',
                        icon: 'https://img.shields.io/badge/Microsoft%20Excel-217346?style=for-the-badge&logo=microsoftexcel&logoColor=white',
                        clicked: false,
                    },
                ],
            },
            {
                title: 'DevOps · Операционные системы',
                subtitle: 'Операционные системы',
                skills: [
                    {
                        name: 'Fedora',
                        icon: 'https://img.shields.io/badge/Fedora-294172?style=for-the-badge&logo=fedora&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Ubuntu',
                        icon: 'https://img.shields.io/badge/Ubuntu-E95420?style=for-the-badge&logo=ubuntu&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Windows',
                        icon: 'https://img.shields.io/badge/Windows-0078D6?style=for-the-badge&logo=windows&logoColor=white',
                        clicked: false,
                    },
                    {
                        name: 'Linux',
                        icon: 'https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black',
                        clicked: false,
                    },
                ],
            },
        ],
    },
};
